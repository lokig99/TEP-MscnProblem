...
bool CMscnSolution::bLoadFromFile(string sFileName)
{
	...
	if(pf_file != NULL)
	{
		...
		//read xd
		if(!b_read_matrix(pf_file, DEF_XD, v_amount_xd))
			return false;

		//read xf 
		if(!b_read_matrix(pf_file, DEF_XF, v_amount_xf))
			return false;

		//read xm 
		if(!b_read_matrix(pf_file, DEF_XM, v_amount_xm))
			return false;
...
{
	return b_set_value(v_amount_xd, iRow, iColumn, dValue);
}

bool CMscnSolution::bSetValueXF(int iRow, int iColumn, double dValue)
{
	return b_set_value(v_amount_xf, iRow, iColumn, dValue);
}

bool CMscnSolution::bSetValueXM(int iRow, int iColumn, double dValue)
{
	return b_set_value(v_amount_xm, iRow, iColumn, dValue);
}
...
bool CMscnSolution::b_fill_safe_xd()
{
	...
	for(size_t i = 0; i < v_amount_xd.size(); ++i)
	{
		...
		for(size_t j = 0; j < v_amount_xd[0].size(); ++j)
		{
			...
			if(v_amount_xd[i][j] < ROUND_TO_ZERO_BELOW)
				v_amount_xd[i][j] = 0.0;

			d_deliverers_prod_cap_left -= v_amount_xd[i][j];
		}
	}
...
bool CMscnSolution::b_fill_safe_xf()
{
	...
	for(size_t i = 0; i < v_amount_xf.size(); ++i)
	{
		double d_products_left = d_resources_amount(v_amount_xd, i);
		...
		for(size_t j = 0; j < v_amount_xf[0].size(); ++j)
		{
			...
			if(v_amount_xf[i][j] < ROUND_TO_ZERO_BELOW)
				v_amount_xf[i][j] = 0.0;

			d_factory_prod_cap_left -= v_amount_xf[i][j];
		}
	}
...
bool CMscnSolution::b_fill_safe_xm()
{
	...
	for(size_t i = 0; i < v_amount_xm.size(); ++i)
	{
		double d_products_left = d_resources_amount(v_amount_xf, i);
		...
		for(size_t j = 0; j < v_amount_xm[0].size(); ++j)
		{
			...
			if(v_amount_xm[i][j] < ROUND_TO_ZERO_BELOW)
				v_amount_xm[i][j] = 0.0;

			d_magazine_cap_left -= v_amount_xm[i][j];
		}
	}
